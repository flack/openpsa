...
//      logMsg("dtnode.activate(): %o", this);
        ...
        if( opts.persist )
            $.cookie(opts.cookieId+"-active", this.data.key, { path: opts.cookieDir });
...
//      logMsg("dtnode.deactivate(): %o", this);
        if( this.tree.activeNode === this ) {
            ...
            if( opts.persist )
                $.cookie(opts.cookieId+"-active", "", { path: opts.cookieDir });
...
//      logMsg("dtnode.onFocus(%o): %o", event, this);
        ...
        if ( event.type=="blur" || event.type=="focusout" ) {
            ...
            if( opts.persist )
                $.cookie(opts.cookieId+"-focus", null, { path: opts.cookieDir });
            ...
            if( opts.persist )
                $.cookie(opts.cookieId+"-focus", this.data.key, { path: opts.cookieDir });
        }
...
//          logMsg("_changeNodeList: write cookie <%s> = '%s'", cookieName, keyList.join("', '"));
            $.cookie(cookieName, keyList.join(","),{ path: this.options.cookieDir });
        } else {
