    ...
    var DM2AjaxEditorBaseObject = {
        ...
        methods: {
            ...
            prepare_composite: function()
            {
                ...
                var self = this;
                var button = $('#' + this.identifier + '_button');
            ...
            _get_field_input_value: function(field)
            {
                ...
                var self = this;
                var id = field.attr('id');
                var name = id.replace(this.identifier+'_', '');
		...
		if (input.length < 1)
		{
			...
			if (input_child.length > 0)
			{
                ...
                $.each($.dm2.ajax_editor.wysiwygs.configs, function(wysiwyg_name, config){
                    if (typeof(config.className) == 'undefined' || typeof($.dm2.ajax_editor.wysiwygs[wysiwyg_name]) == 'undefined') {
                        return;
                    }
            ...
            _on_form_submit: function(status)
            {
                ...
                {
                    case 'save':
                        var on_submit_text = $.dm2.ajax_editor.strings.on_save_text;
                        break;

                    case 'cancel':
                        var on_submit_text = $.dm2.ajax_editor.strings.on_cancel_text;
                        break;

                    case 'preview':
                        var on_submit_text = $.dm2.ajax_editor.strings.on_preview_text;
                        break;

                    case 'edit':
                        var on_submit_text = $.dm2.ajax_editor.strings.on_edit_text;
                        break;
                ...
                /*
                // Cover the view port with a canvas
                jQuery('<div></div>')
                    .addClass('midcom_helper_datamanager2_form_processing')
                    .css({
                        position: 'absolute',
                        top: 0,
                        left: 0,
                    ...
                    .fadeTo(0, 0, function()
                    {
                        ...
                        jQuery('<span></span>')
                            ...
                            .css({
                                position: 'absolute',
                                top: Math.round(jQuery(window).height() / 2) + 'px',
                                left: '50%',
                                color: '#ffffff'
                            })
                            ...
                            jQuery(this).find('span').css({
                                marginLeft: '-' + Math.round(jQuery(this).find('span').width() / 2) + 'px',
                                marginTop: '20px',
                                zIndex: '10001'
                            });
                    });
            ...
            _parse_response: function(data)
            {
                ...
                if (this.state.current == 'delete') {
                    ...
                    var status = $('deletion', data).find('status').text();
    ...
    var DM2AjaxEditorInline = function()
    {
        return $.extend({}, DM2AjaxEditorBaseObject.methods, {
            ...
            render_toolbar: function()
            {
                ...
                {
                    if ($('#' + this.identifier + '_ajax_toolbar')) {
