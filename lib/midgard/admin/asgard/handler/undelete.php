            ...
            foreach ($child_types as $type => $children) {
                if (   count($children) < 10
                    || isset($_GET['show_children'][$object->guid][$type])) {
                        ...
                        echo "{$prefix}    <tr>\n";
                        echo "{$prefix}        <td class=\"label\" style=\"padding-left: {$child_indent}px\" colspan=\"5\"><a href=\"?show_children[{$object->guid}][{$type}]=1\">" . sprintf($this->_l10n->get('show %s %s children'), count($children), midgard_admin_asgard_plugin::get_type_label($type)) . "</a></td>\n";
                        echo "{$prefix}    </tr>\n";
                    }
            }
