    ...
    */
    class hKit
    {
        ...
        private function getNodeValue($node, $className)
        {
            ...
            if ( is_array($this->att_map)
                 && array_key_exists($className, $this->att_map))
            {
            ...
            if ( is_array($this->callbacks)
                && array_key_exists($className, $this->callbacks))
            {
        ...
        {
            // Note: this has been patched for MidCOM compatibility. Henri Bergius <henri.bergius@iki.fi>
            require_once(MIDCOM_ROOT . "/org/openpsa/httplib/microformats/{$profile}.php");
        }
