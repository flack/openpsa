...
class org_openpsa_mypage_handler_workingon extends midcom_baseclasses_components_handler
{
    ...
    function _handler_set($handler_id, $args, &$data)
    {
        ...
        if (!$stat)
        {
            $_MIDCOM->uimessages->add($this->_l10n->get('org.openpsa.mypage'),  'Failed to set "working on" parameter to "' . $_POST['task'] . '", reason ' . midcom_application::get_error_string(), 'error');
        }
