...
<div class="org_openpsa_projects_projectbroker_prospect">
    ...
    <table class="slots">
        <thead>
            <tr>
                <th><?php echo $_MIDCOM->i18n->get_string('event before', 'org.openpsa.calendar'); ?></th>
                <th><?php echo $_MIDCOM->i18n->get_string('available slot', 'org.openpsa.calendar'); ?></th>
                <th><?php echo $_MIDCOM->i18n->get_string('event after', 'org.openpsa.calendar'); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php
            foreach ($data['slots'] as $k => $slot)
            {
                ...
                {
                    echo '        ' . $_MIDCOM->i18n->get_string('no event', 'org.openpsa.calendar') . "\n";
                }
                echo "    </td>\n";
