...
do
{
    ...
    foreach ($results as $result)
    {
        if (!$result->delete())
        {
            throw new midcom_error('Deleting entry ' . $result->guid . ' failed: ' . midcom_connection::get_error_string());
        }
    }
        ...
        catch (midcom_error $e)
        {
                     ...
                     || midcom_connection::get_error() === MGD_ERR_NOT_EXISTS)
            {
                ...
                foreach ($to_delete as $entry)
                {
                    if (!$entry->delete())
                    {
                        throw new midcom_error('Deleting entry ' . $entry->guid . ' failed: ' . midcom_connection::get_error_string());
                    }
                }
            }
            else
            {
                throw new midcom_error('Unexpected error: ' . midcom_connection::get_error_string());
            }
        }
    }
    $offset += $chunk_size;
