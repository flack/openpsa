 ...
 * @version $Id: misc.php 26626 2010-08-26 17:50:01Z netblade $
...
function mgd_include_snippet_php($path)
{
    ...
    if (empty($code))
    {
        debug_add("mgd_include_snippet_php: Could not find snippet {$path}: ", MIDCOM_LOG_ERROR);
        return false;
    }
 ...
 */
function midcom_generate_urlname_from_string($string, $replacer = "-", $r = 0)
{
    if ($r > 5)
    {
    ...
    return midcom_generate_urlname_from_string($safe, $replacer, $r+1);
}
