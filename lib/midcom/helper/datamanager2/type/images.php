 ...
 * @version $Id: images.php 26643 2010-09-12 11:02:38Z adrenalin $
...
class midcom_helper_datamanager2_type_images extends midcom_helper_datamanager2_type_image
{
     ...
     * @return array
     */
    public static function get_image_guids($object, $field)
    {
        if (!$object->guid)
        {
        ...
        $image_maps = $object->get_parameter('midcom.helper.datamanager2.type.images', "attachment_map_{$field}");
        if (!$image_maps)
        {
            return null;
        }
        ...
        {
            $image_map = explode(':', $image_map);
            if (count($image_map) != 3)
            {
                continue;
            }
