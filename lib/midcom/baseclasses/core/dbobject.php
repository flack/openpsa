...
class midcom_baseclasses_core_dbobject extends midcom_baseclasses_core_object
{
    ...
    /**
     * "Pre-flight" MultiLang checks for delete method
     *
     * Separated so that code can be reused
     *
     * @param midgard_object $object a object to do the ML deletion sanity-checks for.
     * @return boolean indicating whether it's safe to delete this object or not
     **/
    public static function delete_pre_multilang_checks($object)
    {
        // Current language is not master, selective delete to prevent deleting too much of objects.
        // Object will not be deleted if:
        // - it doesn't have a language property at all and its ML parent have content in master language,
        // or if
        // - its language property is not the one requested for deletion

        if (midcom_application::get_lang() === midcom_application::get_default_lang())
        {
            // We are in default language, can return true early
            return true;
        }

        if (   $GLOBALS['midcom_config']['multilang_lang0_langs']
            && in_array(midcom_services_multilang::get_lang(), $GLOBALS['midcom_config']['multilang_lang0_langs']))
        {
            // The multilang lang0 workflow doesn't have the concept of master controlling language
            // Or basically with it all of its languages are masters
            return true;
        }

        if ($_MIDCOM->dbfactory->is_multilang($object))
        {
            if ($object->lang !== $_MIDCOM->i18n->code_to_id(midcom_application::get_lang()))
            {
                // ML object's lang does not match current lang, abort delete
                return false;
            }
        }
        else
        {
            if (   $GLOBALS['midcom_config']['multilang_auto_langs']
                && in_array(midcom_services_multilang::get_lang(), $GLOBALS['midcom_config']['multilang_auto_langs']))
            {
                // The multilang auto workflow doesn't have the concept of master controlling language
                // We must allow delete in that case because otherwise there would be orphans left behind
                return true;
            }
            ...
            $parent_ml_object = midcom_helper_reflector_tree::get_parent($object);
            while (   $parent_ml_object
                   && !$_MIDCOM->dbfactory->is_multilang($parent_ml_object))
            {
                $parent_ml_object = midcom_helper_reflector_tree::get_parent($parent_ml_object);
            }
            if ($parent_ml_object)
            {
                $default_lang = midcom_application::get_default_lang();
                $languages = $parent_ml_object->get_languages();
                ...
                unset($default_lang, $languages, $language);
            }
            unset($parent_ml_object);
        }

        // If nothing else resolves the situation default to true
        return true;
    }
    ...
    public static function delete_pre_checks($object)
    {
        ...
        if (!self::delete_pre_multilang_checks($object))
        {
            debug_push_class($object, __FUNCTION__);
            if (!$_MIDCOM->dbfactory->is_multilang($object))
            {
                $msg = "not multilingual and we are not in default language context";
            }
            else
            {
                $msg = "in wrong language {$object->lang}";
            }
            debug_add("Failed to delete object, object " . get_class($object) . " {$object->guid} is {$msg}.",
                MIDCOM_LOG_ERROR);
    ...
    public static function generate_urlname($object, $titlefield = 'title')
    {
        ...
        /**
         * Keep this as reminder in case we end up needing something like it in the future
         * (Should not happen as now we don't do blind overrides, see #805 & #809)
         *
         * NOTE: This is not reached, see the return above!
         */
        if (   $_MIDCOM->dbfactory->is_multilang($object)
            && !empty($object->name))
        {
             ...
             */
            $fallback_language = midcom_application::get_default_lang();
            if ($object->lang != $_MIDCOM->i18n->code_to_id($fallback_language))
            {
                debug_push_class(__CLASS__, __FUNCTION__);
    ...
    private static function _list_parameters_domain($object, $domain)
    {
        ...
        /*
        $parameters = self::_list_parameters_all($object);
