 ...
 * @package midcom.admin.user
 */
class midcom_admin_user_handler_user_account extends midcom_baseclasses_components_handler
    ...
    public function _handler_edit($handler_id, array $args, array &$data)
    {
        ...
        switch ($data['controller']->process_form()) {
            case 'save':
                ...
                midcom::get()->uimessages->add($this->_l10n->get('midcom.admin.user'), sprintf($this->_l10n->get('person %s saved'), $this->person->name));
                return new midcom_response_relocate("__mfa/asgard_midcom.admin.user/edit/{$this->person->guid}/");
        ...
        if ($this->account->get_username() !== '') {
            $data['asgard_toolbar']->add_item(array(
                MIDCOM_TOOLBAR_URL => "__mfa/asgard_midcom.admin.user/account/delete/{$this->person->guid}/",
        ...
        $this->add_breadcrumb("__mfa/asgard_midcom.admin.user/edit/" . $this->person->guid . '/', $this->person->name);
    ...
    public function _handler_delete($handler_id, array $args, array &$data)
    {
        ...
        switch ($data['controller']->process_form()) {
            case 'delete':
                $this->account->delete();
                // Show confirmation for the user
                midcom::get()->uimessages->add($this->_l10n->get('midcom.admin.user'), sprintf($this->_l10n->get('account for %s deleted'), $this->person->name));
                //fall-through
            case 'cancel':
                return new midcom_response_relocate("__mfa/asgard_midcom.admin.user/edit/{$this->person->guid}/");
        }
