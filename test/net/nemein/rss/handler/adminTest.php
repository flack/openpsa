...
class net_nemein_rss_handler_adminTest extends openpsa_testcase
{
    ...
    {
        $data = $this->run_handler(self::$topic, array('__feeds', 'rss', 'subscribe'));
        $this->assertEquals('____feeds-rss-feeds_subscribe', $data['handler_id']);
    }

    public function test_handler_edit()
    {
        $feed = $this->create_object('net_nemein_rss_feed_dba', array('node' => self::$topic->id));
        $data = $this->run_handler(self::$topic, array('__feeds', 'rss', 'edit', $feed->guid));
